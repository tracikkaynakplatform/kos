...
export async function getRegionList(managementClusterName: string, infrastructure: Infrastructure): Promise<any> {
	...
	switch (infrastructure) {
		case Infrastructure.AWS:
			const cli = new AwsCli({
				...
				AWS_REGION: managementCluster.configuration.AWS_REGION ?? "eu-west-1",
				AWS_SECRET_ACCESS_KEY: managementCluster.configuration.AWS_SECRET_ACCESS_KEY,
			});
			return await cli.listRegions();
	}
