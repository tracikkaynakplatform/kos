				...
				<Box sx={{
					...
					<Box sx={{
						...
						<FormControl fullWidth>
							...
							>
								{regions.map(x => (
									<MenuItem key={x.key} value={x.key}>{x.name}</MenuItem>
								))}
							</Select>
						</FormControl>
						<FormControl fullWidth>
							...
							>
								{kubernetesVersions.map(kv => (
									<MenuItem key={kv} value={kv}>{kv}</MenuItem>
								))}
							</Select>
						</FormControl>
						<FormControl fullWidth>
							...
							<Select
								...
								onChange={(e) => setWorkerMachineSize(e.target.value)}
							>
								{machineSizes.map(x => (
						...
						<FormControl fullWidth>
							...
							<Select
								...
								onChange={(e) => setMasterMachineSize(e.target.value)}
							>
								{machineSizes.map(x => (
									<MenuItem key={x.name} value={x.name}>{x.name}</MenuItem>
								))}
					...
					<Box sx={{
						...
						<FormControl fullWidth>
							...
							<Select
								...
								onChange={(e) => setMasterImageId(e.target.value)}
							>
								{customImages.map(x => (
						...
						<FormControl fullWidth>
							...
							<Select
								...
								onChange={(e) => setWorkerImageid(e.target.value)}
							>
								{customImages.map(x => (
									<MenuItem key={x.id} value={x.id}>{x.name}</MenuItem>
								))}
